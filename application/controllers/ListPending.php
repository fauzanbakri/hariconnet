       ...
       $q = $this->db->query("SELECT DISTINCT tiket.tim FROM tiket LEFT JOIN olt ON olt.idOlt = tiket.idOlt WHERE tiket.status NOT IN ('CLOSED', 'SOLVED (ICRM OPEN)') ORDER BY tim ASC;")->result();
        foreach($q as $kab){
            if ($kab->tim!=''){
                ...
                $qn = $this->db->query("SELECT DISTINCT kabupaten FROM tiket LEFT JOIN olt ON olt.idOlt = tiket.idOlt WHERE tiket.tim='$kab->tim' AND tiket.status NOT IN ('CLOSED', 'SOLVED (ICRM OPEN)') ORDER BY kabupaten DESC;")->result();
                foreach($qn as $d){
                    echo "=====".$d->kabupaten."=====<br><br>";
                    $qx = $this->db->query("SELECT * FROM tiket LEFT JOIN olt ON olt.idOlt=tiket.idOlt WHERE kabupaten='$d->kabupaten' AND tim='$kab->tim' AND tiket.status NOT IN ('CLOSED', 'SOLVED (ICRM OPEN)') ORDER BY prioritas, tanggal ASC;")->result();
                    $no = 1;
                    foreach ($qx as $c){
                        echo $no.".
